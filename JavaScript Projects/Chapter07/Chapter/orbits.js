...
function selectDate(event) {
	...
	if (callerElement.innerHTML === "") {
		// cell contains no date, so dont close the calendar
		document.getElementById("cal").style.display = "block";
		return false;
	}
	dateObject.setDate(callerElement.innerHTML);
	var fullDateToday = new Date();
	var dateToday = Date.UTC(fullDateToday.getFullYear(), fullDateToday.getMonth(),
	 fullDateToday.getDate());
	var selectDate = Date.UTC(fullDateToday.getFullYear(), fullDateToday.getMonth(),
	 fullDateToday.getDate());
...
function updateTotalCost() {
	...
	var shortMonlthyCost = monthlyCost.toFixed(0);
	document.getElementById("singleLabel").innerHTML = "Single payment of $" + totalCost.toLocaleDateString();
	document.getElementById("multipleLabel").innerHTML = "60 monthly payments of $" + shortMonlthyCost.toLocaleDateString();
